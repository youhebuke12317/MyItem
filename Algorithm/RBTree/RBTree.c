 ...
 * @auther		Auther
 * @version		1.0.0.1
 * @date			2018年04月29日 星期日 19时03分47秒
 */
...
static void rbtree_delete_fixup(RBRoot *root, Node *node, Node *parent)
{
	...
	while ((node == NULL || rb_is_black(node)) && node != root->node)
	{
		if (parent->left == node)
		{
			other = parent->right;
			if (rb_is_red(other)) {
				...
				// Case 1: x的兄弟w是红色的  
				rb_set_black(other);				// 将x的兄弟节点设为“黑色”
				rb_set_red(parent);					// 将x的父节点设为“红色”
				rbtree_left_rotate(root, parent);	// 对x的父节点进行左旋
				other = parent->right;				// 左旋后，重新设置x的兄弟节点
			}
			...
			if ((!other->left || rb_is_black(other->left)) &&
				...
				rb_set_red(other);					// 将x的兄弟节点设为“红色”
				node = parent;						// 设置“x的父节点”为“新的x节点”
				...
				if (!other->right || rb_is_black(other->right)) {
					...
					rb_set_black(other->left);			// 将x兄弟节点的左孩子设为“黑色”
					rb_set_red(other);					// 将x兄弟节点设为“红色”
					rbtree_right_rotate(root, other);	// 对x的兄弟节点进行右旋
					other = parent->right;				// 右旋后，重新设置x的兄弟节点
				}

				// Case 4: x的兄弟w是黑色的；并且w的右孩子是红色的，左孩子任意颜色。
				rb_set_color(other, rb_color(parent));	// 将x父节点颜色 赋值给 x的兄弟节点
				rb_set_black(parent);					// 将x父节点设为“黑色”
				rb_set_black(other->right);				// 将x兄弟节点的右子节设为“黑色”
				rbtree_left_rotate(root, parent);		// 对x的父节点进行左旋
				node = root->node;						// 设置“x”为“根节点”
				break;
			}
		}
...
void rbtree_delete(RBRoot *root, Node *node)
{
	...
	if ( (node->left != NULL) && (node->right != NULL) ) 
	{
		...
		if (parent == node) {
			...
			// child不为空
			if (child != NULL)	rb_set_parent(child, parent);
