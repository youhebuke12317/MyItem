...
* @auther		Auther
* @version		1.0.0.1
* @date			2018年08月05日 星期日 22时55分47秒
*/
...
// 结点node内对关键字进行二分查找。
int binarySearch(BTNode* node, int low, int high, int Fkey)
{
	int mid;
	while (low<=high) {
		mid = low + (high-low) >> 1;

		if (Fkey<node->key[mid]) 	high = mid-1;
		if (Fkey>node->key[mid]) 	low = mid+1;
		if (Fkey==node->key[mid]) 	return mid;		// 返回下标。
	}
	return 0;										// 未找到返回0.
}
...
void BTree_recursive_remove(BTree* tree, int key)
{
	// B-数的保持条件之一：
...
BTNode* BTree_recursive_search(const BTree tree, int key, int* pos)
{
	int i = 0;
