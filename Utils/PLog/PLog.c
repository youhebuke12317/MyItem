 ...
 *                  ITEM_LOG_DEBUG
 * @param type 日志输出形式
 *                  ITEM_LOG_TYPE_OUTPUT
 *                  ITEM_LOG_TYPE_FILE 
 * */
item_log_t *PLog_init(int level, int type, const char *name, const char *file)
{
#if 0
    if (type & ITEM_LOG_TYPE_OUTPUT) { 
        printf("---------- ITEM_LOG_TYPE_OUTPUT ----------\n"); 
    }
    if (type & ITEM_LOG_TYPE_FILE) {
        printf("----------- ITEM_LOG_TYPE_FILE ------------\n"); 
    }
#endif 

    item_log_t *plog = (item_log_t *)malloc(sizeof(item_log_t));
    if (plog == NULL) {
		printf("item_log_t malloc error\n");
		return NULL;   
	}
    memset(plog, 0, sizeof(item_log_t));

    plog -> type  = type;
    plog -> level = level;
    if (name != NULL)   strcpy(plog -> name, name);

    if ((type & ITEM_LOG_TYPE_FILE) && (file != NULL)) {
        // 系统日志处理
...
{ 
	...
	if (log_info == NULL) {
		if (level & ITEM_LOG_ERR) {
			printf("%s", buf);  
		}

	} else {
		if (log_info -> type & ITEM_LOG_TYPE_OUTPUT) { 
			if (level & log_info -> level) {
				printf("%s", buf);  
			}
		}  

		if (log_info -> type & ITEM_LOG_TYPE_FILE) {
			if (level & log_info -> level) {
				if (level & ITEM_LOG_DEBUG)  
					syslog(LOG_DEBUG, "%s", buf);
				else if (level & ITEM_LOG_INFO) 
					syslog(LOG_INFO, "%s", buf);  
				else if (level & ITEM_LOG_WARN)
					syslog(LOG_WARNING, "%s", buf);  
				else if (level & ITEM_LOG_ERR)
...
static void init_log(int level, const char *name, const char *filename)
{
	...
	//if      (level & ITEM_LOG_DEBUG)   setlogmask(LOG_UPTO(LOG_DEBUG));
	//else if (level & ITEM_LOG_INFO)    setlogmask(LOG_UPTO(LOG_INFO));
	//else if (level & ITEM_LOG_WARN)    setlogmask(LOG_UPTO(LOG_WARNING));
	//else if (level & ITEM_LOG_ERR)     setlogmask(LOG_UPTO(LOG_ERR));
		...
		if(strcmp(line, "}") == 0) {
			...
			if(strcmp(key, "log_level_err") == 0) {			// 日志级别为 ERR
				if (!strcasecmp(value, "ON")) {				// 开启ERR日志级别
					if (*log_level == 0)	*log_level = ITEM_LOG_ERR;
					else	*log_level = *log_level | ITEM_LOG_ERR;
				}
			}
			else if(strcmp(key, "log_level_warn") == 0) {
				if (!strcasecmp(value, "ON")) {
					if (*log_level == 0)	*log_level = ITEM_LOG_WARN;
					else	*log_level = *log_level | ITEM_LOG_WARN;
				}
			}
			else if(strcmp(key, "log_level_info") == 0) {
				if (!strcasecmp(value, "ON")) {
					if (*log_level == 0)	*log_level = ITEM_LOG_INFO;
					else	*log_level = *log_level | ITEM_LOG_INFO;
				}
			}
			else if(strcmp(key, "log_level_debug") == 0) {
				if (!strcasecmp(value, "ON")) {
					if (*log_level == 0)	*log_level = ITEM_LOG_DEBUG;
					else	*log_level = *log_level | ITEM_LOG_DEBUG;
				}
			}
			else if(strcmp(key, "log_model_output") == 0) {
				if (!strcasecmp(value, "ON")) {
					if (*log_model == 0)	*log_model = ITEM_LOG_TYPE_OUTPUT;
					else	*log_model = *log_model | ITEM_LOG_TYPE_OUTPUT;
				}
			}
			else if(strcmp(key, "log_model_file") == 0) {
				if (!strcasecmp(value, "ON")) {
					if (*log_model == 0)	*log_model = ITEM_LOG_TYPE_FILE;
					else	*log_model = *log_model | ITEM_LOG_TYPE_FILE;
				}
			}
				...
				if (log_file != NULL && (strlen(value) < len))	{
					...
					*log_model = ITEM_LOG_TYPE_OUTPUT; 
				}
			}
			else {
